...
function goToPage(target, updateHash = true) {
  ...
  updateUI();
  if (updateHash) history.replaceState(null, '', '#page-' + currentPage);
...
window.addEventListener('message',e=>{
  if(e.data && e.data.type==='book-page') {
    const target = Number(e.data.page);
...
// If a separate page is opened directly, its navigation links can return
// here with #page-N and the correct book page will open.
function openPageFromHash() {
  const match = window.location.hash.match(/^#page-(\d+)$/);
  if (!match) { updateUI(); return; }
  const target = Number(match[1]);
  if (!Number.isInteger(target) || target < 0 || target >= pages.length) { updateUI(); return; }
  for (let i=0; i<target; i++) pages[i].classList.add('flipped');
  for (let i=target; i<pages.length; i++) pages[i].classList.remove('flipped');
  currentPage = target;
  updateUI();
}
